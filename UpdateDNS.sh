...
# Read the current IP from the file before updating
  previous_ipv4=$(cat "./ipv4.txt" 2>/dev/null)
  previous_ipv6=$(cat "./ipv6.txt" 2>/dev/null)
...
for website in "${websites[@]}"; do
  ...
  # Check if the IPv4 update was successful
    if [ "$(echo "$response" | jq -r .success)" = "true" ]; then
      echo -e "\nIPv4 updated successfully for $website"
      log_update "$website IPv4 update successful. Previous IP: $previous_ipv4, New IP: $ipv4"
      echo "$ipv4" > "./ipv4.txt"
    else
      echo -e "\nIPv4 update failed."
  ...
  # Check if the IPv6 update was successful
    if [ "$(echo "$response" | jq -r .success)" = "true" ]; then
      ...
      echo -e "\nIPv6 updated successfully for $website"
      log_update "$website IPv6 update successful. Previous IP: $previous_ipv6, New IP: $ipv6"
      echo "$ipv6" > "./ipv6.txt"
    else
      echo -e "\nIPv6 update failed."
